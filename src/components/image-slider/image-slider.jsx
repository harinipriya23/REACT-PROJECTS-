...
export default function ImageSlider({ url, limit = 9, page = 3 }) {
  ...
  useEffect(() => {
    if (url !== "") fetchingImages(url);
  }, [url]);

  function handlePrevImg() {
    const prevClick = currentSlide === 0 ? image.length - 1 : currentSlide - 1;
    setCurrentSlide(prevClick);
  }

  function handleNextImg() {
    const nextClick = currentSlide === image.length - 1 ? 0 : currentSlide + 1;
